    ...
    try:
        ...
        for i in range(num_generations):
            ...
            try:
                ...
                response_text = output[0]['generated_text'].strip() if output else None
                ...
                with open("response-agent.json", "w", encoding="utf-8") as f:
                    json.dump(results, f, indent=2, ensure_ascii=False)

                # Small delay between generations
                if i < num_generations - 1:  # Don't delay after the last generation
                    time.sleep(2)

            except Exception as e:
                logger.error(f"Error in generation {i+1}: {str(e)}")
                results["failed_generations"] += 1
                # Still add the failed attempt to the results
                generation_result = {
                    "generation_number": i + 1,
                    "error": str(e),
                    "success": False
        ...
        logger.info(f"Results saved to response-agent.json")
